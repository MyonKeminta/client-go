...
func (s *testRegionRequestToThreeStoresSuite) TestStaleReadRetry(c *C) {
	...
	c.Assert(region, NotNil)
	// Retry 1 time.
	c.Assert(failpoint.Enable("github.com/tikv/client-go/v2/mockDataIsNotReadyError", `return(1)`), IsNil)
	...
	c.Assert(region, NotNil)
	// Retry 2 times.
	c.Assert(failpoint.Enable("github.com/tikv/client-go/v2/mockDataIsNotReadyError", `return(2)`), IsNil)
	resp, ctx, err = s.regionRequestSender.SendReqCtx(s.bo, req, region.Region, time.Second, tikvrpc.TiKV)
	c.Assert(err, IsNil)
	c.Assert(resp.Resp, NotNil)
	c.Assert(ctx, NotNil)
	...
	c.Assert(region, NotNil)
	// Retry 3 times.
	c.Assert(failpoint.Enable("github.com/tikv/client-go/v2/mockDataIsNotReadyError", `return(3)`), IsNil)
	resp, ctx, err = s.regionRequestSender.SendReqCtx(s.bo, req, region.Region, time.Second, tikvrpc.TiKV)
	c.Assert(err, IsNil)
	c.Assert(resp.Resp, NotNil)
	c.Assert(ctx, NotNil)
	...
	c.Assert(failpoint.Disable("github.com/tikv/client-go/v2/mockDataIsNotReadyError"), IsNil)
}
