...
func (s *testConfigSuite) TestTxnScopeValue(c *C) {
	c.Assert(failpoint.Enable("github.com/tikv/client-go/v2/config/injectTxnScope", `return("bj")`), IsNil)
	isGlobal, v := GetTxnScopeFromConfig()
	c.Assert(isGlobal, IsFalse)
	c.Assert(v, Equals, "bj")
	c.Assert(failpoint.Disable("github.com/tikv/client-go/v2/config/injectTxnScope"), IsNil)
	c.Assert(failpoint.Enable("github.com/tikv/client-go/v2/config/injectTxnScope", `return("")`), IsNil)
	isGlobal, v = GetTxnScopeFromConfig()
	c.Assert(isGlobal, IsTrue)
	c.Assert(v, Equals, "global")
	c.Assert(failpoint.Disable("github.com/tikv/client-go/v2/config/injectTxnScope"), IsNil)
	c.Assert(failpoint.Enable("github.com/tikv/client-go/v2/config/injectTxnScope", `return("global")`), IsNil)
	isGlobal, v = GetTxnScopeFromConfig()
	c.Assert(isGlobal, IsFalse)
	c.Assert(v, Equals, "global")
	c.Assert(failpoint.Disable("github.com/tikv/client-go/v2/config/injectTxnScope"), IsNil)
